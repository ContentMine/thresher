...
var ScraperBox = function(dir) {
  ...
  if (this.scrapers.length > 0) {
    log.info('Successfully loaded ' + this.scrapers.length + 'scrapers');
  }
}
...
ScraperBox.prototype.getScraper = function(url) {
  log.info('Searching for scraper matching URL: ' + url);
  var matches = [];
  ...
  if (matches.length == 0) {
    log.error('No scraper found for URL: ' + url);
  } else if (matches.length == 1) {
    log.info('Single matching scraper found');
    return matches[0];
  }
  log.info('Multiple matching scrapers found - selecting most specific match');
